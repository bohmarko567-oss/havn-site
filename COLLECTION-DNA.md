...
### Products (3 mains + 1 gift — the structure is fixed)
...
| N°04 | [NAME] | [supliful-slug] | free gift, completes the set | [#hex] | [glyph] |

The three mains must read as a NARRATIVE ARC the film can tell (HAVN v1 = morning→night,
warm→cool). Pick an arc, then colors that travel it. Derive deep + tint per SKU
(deep ≈ field darkened ~35%, tint ≈ field at ~12% over cream).
...
Per-month prices are INTEGERS. Trio per-month = exactly 3× single per-month ON EVERY
TIER, so cart collapse (3 singles → trio) is price-neutral at any supply length.
Tier discounts monotone, motivating toward long supply. Badges UNDERSTATE (never round
a discount up). Anchor = one-time price; sub tiers strike it. Gift SKU: purchasable
solo cheap, auto-free with complete set. Server `_catalog.js` and client tables must
...
Stripe DK 5.25% + $0.28 one-time / +0.7% Billing subs. Ship only if trio sub
contribution ≥ $35/month. Singles are anchors, not acquisition products.
